"""ConvLSTM training entrypoint for Moving MNIST intuitive physics.

Mirrors examples/intuitive_physics/train_simvp.py structure so all three
models are trained under identical conditions (same dataset, same budget, same seed).

Run:
    python -m examples.intuitive_physics.train_convlstm \
...
def run(fname="examples/intuitive_physics/cfgs/convlstm_train.yaml", cfg=None,
    ...
    if folder is not None:
        ...
        exp_dir = get_unified_experiment_dir(
            example_name="intuitive_physics_convlstm",
            sweep_name=get_default_dev_name(),
            exp_name=get_exp_name("convlstm", cfg),
            seed=cfg.meta.seed,
        )
    ...
    setup_wandb(
        project="eb_jepa",
        config={"example": "convlstm", **OmegaConf.to_container(cfg, resolve=True)},
        run_dir=exp_dir,
        run_name=exp_dir.name,
        tags=["convlstm", f"seed_{cfg.meta.seed}"],
    ...
    model = build_convlstm(cfg).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    logger.info(f"ConvLSTM parameters: {n_params:,}")
